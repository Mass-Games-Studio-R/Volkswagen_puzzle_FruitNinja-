...
let uclass = {
  exists: function (elem, className) {
    let p = new RegExp("(^| )" + className + "( |$)");
    return elem.className && elem.className.match(p);
  },
  add: function (elem, className) {
    if (uclass.exists(elem, className)) {
      return true;
    }
...
let loadedImages = 0;
let imageArray = new Array("dl_loading.gif");
...
const onResize = () => {
  ...
  internalDiv.style.top =
    content.offsetHeight / 2 - internalDiv.offsetHeight / 2 + "px";
  internalDiv.style.left =
    content.offsetWidth / 2 - internalDiv.offsetWidth / 2 + "px";
  ...
  Draggable.create(buttons, {
    ...
    onDragEnd: function (e) {
      ...
      if (checkCollisions(e.target, gameCar)) {
        ...
        gsap.to([`#carPiece_${id}`, `#message_${id}`], {
          duration: 0.3,
          opacity: 1,
        });
        ...
        if (score === 6) {
          ...
          gsap.to(".car_part:not(#carComplete)", {
            duration: 0.3,
            delay: 0.3,
            opacity: 0,
          });
          ...
          tl.to(
            "#endFrame",
            { duration: 0.5, pointerEvents: "all", opacity: 1, delay: 2 },
            0
          ).from(
            [".endFrame_logo", ".endFrame_txt", ".cta"],
            { stagger: 0.25, duration: 1.25, opacity: 0, y: "-=20" },
            "-=0.2"
          );
        }
      } else {
...
const checkCollisions = (piece, car) => {
  ...
  return (
    carBox &&
    pieceBox.x + pieceBox.width * 0.25 < carBox.x + carBox.width &&
    pieceBox.x + pieceBox.width * 0.75 > carBox.x &&
...
const gameEnd = () => {
  ...
  tl.to(
    "#endFrame",
    { duration: 0.5, pointerEvents: "all", opacity: 1, delay: 3 },
    3
  ).from(
    [".endFrame_logo", ".endFrame_txt", ".cta"],
    { stagger: 0.25, duration: 1.25, opacity: 0, y: "-=20" },
